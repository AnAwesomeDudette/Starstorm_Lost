...
callback.register("onNPCDeath", function(actor)
	for _, func in pairs(onNPCDeath) do
		pcall(func, actor)
	end
end)
...
local funcs = {}
local syncInst = net.Packet.new("SSInstSpawn", function(player, object, x, y, funcid, instid, other)
...
function syncCreate(object, x, y, id, otherv)
	if net.host then
		...
		syncInst:sendAsHost(net.ALL, nil, object, x, y, id, instid, other)
	end
end
...
function makeTamed(actor, parent) -- this function sucks bad
	...
	updateNpcItemVars(actor, parent)
